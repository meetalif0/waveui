...
typedef BarItemEnumeratorCallback = void Function(
    int barBundleIndex,
    WaveProgressBarBundle barBundle,
    int barGroupIndex,
    WaveProgressBarItem barItem);

/// 点击柱状数据的回调
typedef WaveProgressBarChartSelectCallback = void Function(
    WaveProgressBarItem? barItem);
...
/// WaveProgressBarBundle 数据图表的数据集
/// 每一个bundle对应一组数据
class WaveProgressBarBundle {
  final List<WaveProgressBarItem> barList;
...
class WaveProgressBarChartPainter extends CustomPainter {
  /// 柱状图的样式
  ...
  final WaveProgressBarItem? selectedBarItem;
  final WaveProgressBarChartSelectCallback? waveProgressBarChartSelectCallback;
  ...
  WaveProgressBarChartPainter(
      ...
      this.waveProgressBarChartSelectCallback});
  ...
  void _prepareData(Canvas canvas, Size size) {
    ...
    if (0 != this.barMaxValue) {
      ...
      this.barBundleList.forEach((WaveProgressBarBundle barbundle) {
        barbundle.barList.forEach((WaveProgressBarItem barItem) {
    ...
    this.barItemEnumerator((int barBundleIndex, WaveProgressBarBundle barBundle,
        int barGroupIndex, WaveProgressBarItem barItem) {
  ...
  void barItemEnumerator(BarItemEnumeratorCallback barItemEnumeratorCallback) {
    ...
    for (int barBundleIndex = 0;
        ...
        barBundleIndex++) {
      WaveProgressBarBundle barBundle = this.barBundleList[barBundleIndex];
      int barNumber = barBundle.barList.length;
      for (int barGroupIndex = 0; barGroupIndex < barNumber; barGroupIndex++) {
        WaveProgressBarItem barItem = barBundle.barList[barGroupIndex];
  ...
  bool? hitTest(Offset position) {
    if (this.waveProgressBarChartSelectCallback != null) {
      this.barItemEnumerator((int barBundleIndex,
          WaveProgressBarBundle barBundle,
          int barGroupIndex,
          WaveProgressBarItem barItem) {
  ...
  @override
  bool shouldRepaint(WaveProgressBarChartPainter oldDelegate) {
    return true;
  }
}
