...
import 'package:waveui/src/components/charts/broken_line/wave_line_data.dart';
import 'package:waveui/src/components/charts/broken_line/wave_line_painter.dart';
import 'package:waveui/src/components/charts/broken_line/wave_line_y_painter.dart';
import 'package:flutter/material.dart';

/// 适用于需要折线图，曲线图的场景
/// 支持数据过多时左右滑动查看数据
/// 支持 X、Y 轴自定义展示文本内容
class WaveBrokenLine extends StatefulWidget {
  /// 表格宽高
  final Size size;

  /// 绘制线条的参数内容
  final List<WavePointsLine> lines;
  ...
  /// y轴左侧刻度显示，不传则没有
  final List<WaveDialItem>? yDialValues;

  /// x 轴刻度
  final List<WaveDialItem>? xDialValues;
  ...
  @override
  State<StatefulWidget> createState() => WaveBrokenLineState();
}

class WaveBrokenLineState extends State<WaveBrokenLine> {
  int pointSelectIndex = -1;
  int lineSelectIndex = -1;

  @override
  Widget build(BuildContext context) {
    var _lineWithXPainter = WaveLinePainter(
      widget.lines,
  ...
  Widget _buildTouchTipWidget(WavePointData pointData) {
    Widget touchTipWidget;
    WaveLineTouchData? selectLinePoint = pointData.lineTouchData;
