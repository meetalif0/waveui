...
///对绘制的效果也支持一定程度上的定制，参考[WaveRadarChartDataProvider],[WaveRadarChartStyle]。
class WaveRadarChart extends MultiChildRenderObjectWidget {
  ...
  ///提供绘制雷达图所需要的数据
  final WaveRadarChartDataProvider provider;
  ...
  static const List<WaveRadarChartStyle> defaultRadarChartStyles = [
    WaveRadarChartStyle(
  ...
  ///The default style named constructor which is designed with WaveUI style.
...
class WaveRadarChartParentData extends ContainerBoxParentData<RenderBox> {}
...
class RenderRadarChart extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox, WaveRadarChartParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, WaveRadarChartParentData> {
  Paint _radarPainter;
  Paint _axisPainter;
  ...
  WaveRadarChartDataProvider _dataProvider;
  ...
  RenderRadarChart({
    ...
    required WaveRadarChartDataProvider provider,
  ...
  WaveRadarChartDataProvider get dataProvider => _dataProvider;

  set dataProvider(WaveRadarChartDataProvider value) {
  ...
  void setupParentData(RenderBox child) {
    if (child.parentData is! WaveRadarChartParentData) {
      child.parentData = WaveRadarChartParentData();
    }
  }
  ...
  double _getIntrinsicDimensionHorizontal(
    ...
    int i = 0;
    while (child != null) {
      final WaveRadarChartParentData childParentData =
          child.parentData as WaveRadarChartParentData;
  ...
  void _drawRadar(Canvas canvas) {
    ...
    for (int radarIndex = 0; radarIndex < radarCount; radarIndex++) {
      WaveRadarChartStyle radarStyle = _dataProvider.getRadarStyle(radarIndex);
      _radarPainter
        ..isAntiAlias = true
...
///The default WaveRadarChartDataProvider which using the default designed style.
class DefaultRadarProvider extends WaveRadarChartDataProvider {
  ...
  @override
  WaveRadarChartStyle getRadarStyle(int radarIndex) {
    if (radarIndex >= WaveRadarChart.defaultRadarChartStyles.length) {
      return WaveRadarChartStyle(
        strokeColor: Color(0xff0984f9),
        areaColor: Color(0x1a2196F3),
        dotted: true,
        dotColor: Color(0xff0984f9),
    ...
    return WaveRadarChart.defaultRadarChartStyles[radarIndex];
  }
