...
import 'package:waveui/src/components/charts/radar_chart.dart';
...
///   每层layer支持自定义图案绘制，绘制参考[WaveFunnelLayerPainter]类定义
class WaveFunnelChart extends MultiChildRenderObjectWidget {
  ///漏斗的最大宽度，一般是漏斗最上层的宽度
  ...
  ///用于每层layer的绘制，可以继承[WaveFunnelLayerPainter]对每层的绘制进行定制
  final WaveFunnelLayerPainter layerPainter;

  ///WaveUI风格的每层layer预设的颜色值，按顺序使用颜色值。实际层数超过该数量需要自行定义。
  ...
  ///漏斗图默认WaveUI风格的命名构造函数，[layerCount]不能大于[defaultLayerColors.length]。
  WaveFunnelChart.defaultStyle({
    Key? key,
    required this.layerCount,
    required this.markerCount,
    required MarkerBuilder builder,
    ...
    this.layerMargin = 0,
    this.childOffset = Offset.zero,
  })  : this.layerPainter = WaveDefaultFunnelLayerPainter(),
        this.shape = FunnelShape.leftAndRight,
        this.alignment = MarkerAlignment.right,
...
class WaveFunnelChartParentData extends ContainerBoxParentData<RenderBox> {}

abstract class RenderFunnelChart extends RenderBox
    with
        ContainerRenderObjectMixin<RenderBox, WaveFunnelChartParentData>,
        RenderBoxContainerDefaultsMixin<RenderBox, WaveFunnelChartParentData> {
  RenderFunnelChart({
    ...
    required WaveFunnelLayerPainter layerPainter,
  ...
  WaveFunnelLayerPainter get layerPainter => _layerPainter;

  WaveFunnelLayerPainter _layerPainter;

  set layerPainter(WaveFunnelLayerPainter value) {
  ...
  void setupParentData(RenderBox child) {
    if (child.parentData is! WaveFunnelChartParentData) {
      child.parentData = WaveFunnelChartParentData();
    }
  }
...
///漏斗图RenderObject
class WaveFunnelRender extends RenderFunnelChart {
  ...
  double? getIntrinsicDimensionHorizontal(
    ...
    if (_alignment == MarkerAlignment.center) {
      while (child != null) {
        final WaveFunnelChartParentData childParentData =
            child.parentData as WaveFunnelChartParentData;
      ...
      while (child != null) {
        ...
        num++;
        WaveFunnelChartParentData childParentData =
            child.parentData as WaveFunnelChartParentData;
        child = childParentData.nextSibling;
      }
    }
...
///漏斗每层绘制接口类，自定义每层绘制继承该类。示例查看[WaveDefaultFunnelLayerPainter]。
...
///漏斗图默认LayerPainter,在漏斗每层layer中间绘制文案，每层的颜色值使用WaveUI预设的颜色。
class WaveDefaultFunnelLayerPainter extends WaveFunnelLayerPainter {
  ///每层漏斗文案TextStyle
  ...
  List<ui.Color> getLayerColors(int layerIndex) {
    if (layerIndex >= WaveFunnelChart.defaultLayerColors.length) {
      //超过最大支持的layer层数
      return [Colors.white];
    }
    return [WaveFunnelChart.defaultLayerColors[layerIndex]];
  }
