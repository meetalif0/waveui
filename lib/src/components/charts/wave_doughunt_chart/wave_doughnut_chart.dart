...
/// WaveDoughnutDataItem 饼状图展示所使用的数据
/// 设置数据的数值、标题、颜色
class WaveDoughnutDataItem {
  /// 当前区域值
  final double value;
...
/// 选中扇形区域后执行的回调
typedef WaveDoughnutSelectCallback = void Function(
    WaveDoughnutDataItem? selectedItem);

class WaveDoughnut extends CustomPainter {
  ///圆心位置
  late Offset circleCenter;

  /// 选中的区域
  final WaveDoughnutDataItem? selectedItem;

  /// 选中区域回调
  final WaveDoughnutSelectCallback? waveDoughnutSelectCallback;
  ...
  WaveDoughnut(
      ...
      this.waveDoughnutSelectCallback}) {
...
/// WaveDoughnutChart 圆形数据展示组件
/// 可选择使用圆形、环形来展示数据所占总数的百分比
class WaveDoughnutChart extends StatelessWidget {
  /// 宽度。默认值 0
  final double width;
  ...
  /// 选中项目时候的回掉
  final WaveDoughnutSelectCallback? selectCallback;
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: CustomPaint(
        ...
        foregroundPainter: WaveDoughnut(
            data: this.data,
            ...
            waveDoughnutSelectCallback: this.selectCallback),
      ),
    );
  }
}
