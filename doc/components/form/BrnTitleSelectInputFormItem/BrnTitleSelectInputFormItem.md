---
title: WaveTitleSelectInputFormItem
group:
  title: Form
  order: 12
---
...
<img src="./img/WaveTitleSelectInputFormItemIntro.png" style="zoom:50%;" />
...
| type | Stirng | 录入项类型，主要用于录入类型页面框架中 | 否 | WaveInputItemType.textInputTitleSelectType | 外部可根据此字段判断表单项类型 |
...
| inputType | String | 指定键盘类型 | 否 | WaveInputType.TEXT | 详见**WaveInputType**类，注意：无法通过指定键盘类型确保输入。比如不能通过指定数字键盘确保用户只输入数字。如果有要求用户只输入特定字符的需求请使用**inputFormatters**参数 |
...
![](./img/WaveTitleSelectInputFormItemDemo1.png)
...
WaveTitleSelectInputFormItem(
  ...
  onTitleSelected: (String title, int index) {
    if ('座机' == title && controller.text.length > 5) {
      WaveToast.show('输入长度不符合要求', context);
...
![](./img/WaveTitleSelectInputFormItemDemo2.png)
...
WaveTitleSelectInputFormItem(
  ...
  onTitleSelected: (String title, int index) {
    WaveToast.show("点击触发回调_${title}_${index}_onChanged", context);
  },
)
```
