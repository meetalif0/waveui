---
title: WaveTextBlockInputFormItem
group:
  title: Form
  order: 12
---
...
![](./img/WaveTextBlockInputFormItemIntro.png)
...
| type | Stirng | 录入项类型，主要用于录入类型页面框架中 | 否 | WaveInputItemType.textBlockInputType | 外部可根据此字段判断表单项类型 |
...
| inputType | String? | 输入内容类型，指定键盘类型，参见 `WaveInputType` | 否 | 无 | 详见**WaveInputType**类，注意：无法通过指定键盘类型确保输入。比如不能通过指定数字键盘确保用户只输入数字。如果有要求用户只输入特定字符的需求请使用**inputFormatters**参数 |
...
![](./img/WaveTextBlockInputFormItemDemo1.png)
```dart
WaveTextBlockInputFormItem(
...
```
### 效果2：全功能样式
![](./img/WaveTextBlockInputFormItemDemo2.png)
...
![](./img/WaveTextBlockInputFormItemDemo3.png)
```dart
WaveTextBlockInputFormItem(
  controller: TextEditingController()..text = "hello",
  prefixIconType: WavePrefixIconType.add,
...
![](./img/WaveTextBlockInputFormItemDemo4.png)
```dart
WaveTextBlockInputFormItem(
  controller: TextEditingController()..text = "hello",
  prefixIconType: WavePrefixIconType.add,
