...
关联组件： [WaveGallerySummaryPage](../widgets/wave-gallery-summary-page)

## 一、效果总览

![image-20211101121347565](./img/WaveGalleryDetailPageIntro.png) 
...
1. 如何选择优先进入大图模式（WaveGalleryDetailPage）还是缩略图模式（WaveGallerySummaryPage）：根据实际需求，若用户点开图片的动机是查看图片细节，则进入大图模式。若用户希望在众多图片中快速定位某张图片，则进入缩略图模式。
2. 大图模式下，图片信息展示区的展示内容可根据实际需求配置。
3. 在图片数量较多时，可能引起OOM，可以自行继承**WavePhotoItemConfig**，将其中的图片展示组件替换为自定义图片组件。
4. 当所有图片放在 allConfig 中的一个 Item 中时，不显示 TabBar，如：查看图片，查看视频，pdf(自行扩展配置接口**WaveBasicItemConfig**)
5. WavePhotoItemConfig 中**bottomCardModel** 用于控制图片描述是否可以展开
...
| controller | WaveGalleryController | 控制图片查看刷新，一般在图片数据变化后调用 | 否 | |
...
class WavePhotoGroupConfig extends WaveBasicGroupConfig {  
  ...
  //通过url列表生成配置  
  WavePhotoGroupConfig.url({this.title, @required this.urls}) {  
    configList = List();  
    urls.forEach((item) => configList.add(WavePhotoItemConfig(url: item)));  
  }  
  //自定义每一项配置  
  WavePhotoGroupConfig({this.urls, this.title, this.configList});  
}
```
...
WavePhotoItemConfig({  
  ...
  //占位图  
  this.placeHolder =  
      "packages/${WaveUIStrings.flutterPackageName}/assets/icons/grey_place_holder.png",  
  //单击事件回调  
  this.onTap,  
  //双击事件回调  
  this.onDoubleTap,  
...
<img src="./img/WaveGalleryDetailPageDemo1.jpg" style="zoom:50%;" />  
...
Navigator.push(context, MaterialPageRoute(  
  builder: (BuildContext context) {  
    return WaveGalleryDetailPage(   allConfig: allConfig,   initGroupId:0,   initIndexId:4,);  
  },  
));  
...
![customhandle_filter](./img/WaveGalleryDetailPageDemo3.gif)  
