...
| headerPadding           | EdgeInsets?                    | header 部分的 padding，只有居中样式支持设置                  | 否           | `headerType=WaveAppraiseHeader.center`时，为 `EdgeInsets.only(top: 20, bottom: 20)`，其余为 `EdgeInsets.only(left: 20, top: 16, right: 16, bottom: 20)` |
| titleMaxLines           | int                            | 标题的最大行数                                               | 否           | 1                                                            |
| onCancel                | WaveAppraiseCloseClickCallBack? | 点击关闭按钮时的回调                                         | 否           | 无                                                           |
| count                   | int                            | `type=WaveAppraiseType.Star`时所需五角星的个数                | 否           | 5                                                            |
| iconDescriptions        | `List<String>`                 | 点击表情时的文案。`type=WaveAppraiseType.Star`时，该列表的数据个数不能比 count 少。`type=WaveAppraiseType.Emoji`时，list 长度应为 5，不足 5 个时请在对应位置补空字符串 | 否           | ['不好', '还行', '满意', '很棒', '超惊喜']                   |
...
| iconClickCallback       | WaveAppraiseIconClick?          | 点击打分时的回调                                             | 否           | 无                                                           |
| inputTextChangeCallback | WaveInputTextChangeCallback?    | 输入框内容改变的回调                                         | 否           | 无                                                           |
| tagSelectCallback       | WaveAppraiseTagClick?           | 选择标签时的回调                                             | 否           | 无                                                           |

```dart
const WaveAppraiseConfig({
