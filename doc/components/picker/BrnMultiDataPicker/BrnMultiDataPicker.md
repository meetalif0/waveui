...
![](./img/WaveMultiDataPickerIntro.png)
...
| delegate | WaveMultiDataPickerDelegate | 滚动组件每一列、每一行的具体内容设置，自定义delegate继承这个类，实现具体方法。 | 是 |  |
...
| createItemWidget | WaveMultiDataPickerCreateWidgetCallback? | 返回自定义itemwidget | 否 |  |
| sync | bool | 是否复位数据位置 | 否 | true |
| themeData | WavePickerConfig? | Picker配置类，配置详情见WavePickerConfig | 否 |  |
...
注意示例代码中，Wave1RowDelegate、Wave2RowDelegate 需要自行实现。参考 Demo 代码。

### 效果1：单列滚动选择

![](./img/WaveMultiDataPickerDemo1.png)
...
![](./img/WaveMultiDataPickerDemo2.png)

```dart
WaveMultiDataPicker(
  context: context,
  title: '委托来源',
  delegate: Wave2RowDelegate(firstSelectedIndex: 1, secondSelectedIndex: 0),
  confirmClick: (list) {
    WaveToast.show(list.toString(), context);
  },
).show();
...
![](./img/WaveMultiDataPickerDemo3.png)
...
WaveMultiDataPicker(
  ...
  context: context,
  title: '委托来源',
  delegate: Wave2RowCustomDelegate(firstSelectedIndex: 1, secondSelectedIndex: 0),
  confirmClick: (list) {
    WaveToast.show(list.toString(), context);
  },
).show();
...
class Wave2RowCustomDelegate implements WaveMultiDataPickerDelegate {
  ...
  Wave2RowCustomDelegate({this.firstSelectedIndex = 0, this.secondSelectedIndex = 0});
...
![](./img/WaveMultiDataPickerDemo4.png)
