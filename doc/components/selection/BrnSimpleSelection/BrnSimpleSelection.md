...
| onSimpleSelectionChanged | WaveSimpleSelectionOnSelectionChanged | 选择回调 | 是 |  |
...
WaveSimpleSelection.radio(
  ...
  onSimpleSelectionChanged: (
    ...
    WaveToast.show(filterParams.map((e) => e.value).toList().join(','), context);
  },
)
```
```dart
